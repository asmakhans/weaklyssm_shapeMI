...
def calculate_PCA_analysis(dt_path, particle_path, num_modes):
    ...
    def worker_gen(i):
        ...
        np.savetxt("tmp/p"+str(i)+".particles", current_particles)
        ...
        os.remove("tmp/p"+str(i)+".vtk")
        os.remove("tmp/p"+str(i)+".particles")
    ...
    all_gens = Parallel(n_jobs=20)(delayed(worker_gen)(i) for i in tqdm(range(len(all_particles)), desc="Iter Gen: "))
    ...
    def worker_spec(i):
        ...
        np.savetxt("tmp/p"+str(i)+".particles", sampled_points)
        execCommand = ["shapeworks", 
                    "warp-mesh", "--reference_mesh", template_mesh_new,
                    "--reference_points", template_particles_new,
                    "--target_points", "tmp/p"+str(i)+".particles" ]
    ...
    all_specs = Parallel(n_jobs=30)(delayed(worker_spec)(i) for i in tqdm(range(100), desc="Iter Spec: "))
