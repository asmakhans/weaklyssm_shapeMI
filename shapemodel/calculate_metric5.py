...
def calculate_PCA_analysis(num_modes):
    ...
    gen_train = 0#getGeneralization_train(num_modes, point_embedder)
    ...
    #     json.dump(info, f, indent=4)
    spec = 0 #np.mean(np.array(all_specs))
...
out = np.array(out)
np.savez(os.path.join(path, "groomed_test", "stats_gen.npz"), comp=out[:,0], spec=out[:,1], gen=out[:,2], gen_train=out[:,3])
tqdm.write("---- Done ----")
